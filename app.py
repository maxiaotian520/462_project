...
def create():
    ...
    time.sleep(1)
    cursor.execute(""" CREATE TABLE IF NOT EXISTS users(name TEXT, email TEXT, password BLOB, genre TEXT) """)
    db.commit()

#-- Insert function add new record to database
def insert(name, email, password, genre):
    db = sq.connect("site_db")
    cursor = db.cursor()
    cursor.execute("""INSERT INTO users (name, email, password, genre) VALUES(?,?,?,?)""",(name,email,password,genre))
    db.commit()
    db.close()
...
def register_success():
    ...
    if request.method == "POST":
        ...
        if check_data(email):
            ...
            print(password)
            print(genre)
            insert(name, email, password, genre)
